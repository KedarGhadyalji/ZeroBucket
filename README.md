...
- **`tier_to_object_storage()` locks the whole database file, not just
  ...
  locking at all (it's a single-writer database); this backend uses
  `BEGIN IMMEDIATE` to preserve the same "failed upload leaves the row
  completely untouched" safety guarantee Postgres's `SELECT ... FOR
UPDATE`-based version has, but `BEGIN IMMEDIATE` is a coarser lock --
  every other write to every other row blocks until the upload
...
- **Sync `get_stream()` survives a concurrent delete mid-stream,
  ...
  read transaction gives it a consistent WAL-mode snapshot of the row
...
- **No `RETURNING`.** MariaDB 10.5+ supports it; real MySQL 8.0 does
  ...
  `delete_many()`/`tier_to_object_storage()` use `SELECT ... FOR
...
- **`tier_to_object_storage()`'s row lock is a REAL improvement over
  ...
  per-row locking (`SELECT ... FOR UPDATE`), so tiering one image
  never blocks writes to any other row -- exactly matching Postgres.
  SQLite has no row-level locking at all and has to fall back to
  locking the WHOLE database file (`BEGIN IMMEDIATE`) for the same
...
│   │   │       ├── mysql.py          # sync MySQL/MariaDB implementation (in progress)
